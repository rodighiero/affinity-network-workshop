...
- **Layout toggle** — an **Embedding layout** switch (`layoutMode` `'force'`/`'embedding'`) with a **Dimensions** sub-slider (`embed-dims`, 1D/2D). Force mode is the usual live simulation; embedding mode pins each node at its position in a faithful projection of the vectors (`computeProjection` = classical MDS / PCA: top-2 eigenvectors of the centred Gram matrix via `topEigen` power-iteration + deflation, dependency-free), so on-screen distance reflects real embedding distance. 2D spreads the clips across the plane; 1D lays them on a single horizontal axis (the first principal component, `applyEmbedding` fixing `y` to the stage centre). `refreshPositions()` (factored out of `ticked`) redraws the pinned layout; the force-only sliders (repulsion/gravity/spacing/link distance) are disabled/dimmed via `setForceControlsEnabled`, while threshold still re-draws edges. Links stay drawn over the projected points.
...
- No keyword/edge-keyword layer (audio has no vocabulary); the Network page's TF-IDF code is dropped here. Labels are the clip filenames, with the Network page's **Label length** (`shortenTitle`/`displayName`) and **Label size** controls kept. CLAP pads/truncates to a fixed ~10 s window, so short clips work best.
